...
from .visualization import count_d
...
class Blank(Entity):
    ...
    def live(self):
        ...
        #if random.random() <= 0.004 and self.x > 50 and self.y > 30:
        #    self._container.append(substances.Substance())
        #if random.random() <= 0.004 and self.x < 10 and self.y > 30:
        #    self._container.append(substances.Substance())
        #if random.random() <= 0.004 and self.x < 10 and self.y < 10:
        #    self._container.append(substances.Substance())
        #if random.random() <= 0.004 and self.x > 50 and self.y < 10:
        #    self._container.append(substances.Substance())
        #if random.random() <= 0.4 and self.x > 28 and self.x < 33 and self.y > 18 and self.y < 23:
        #    self._container.append(substances.Substance())
        if random.random() <= 0.0008:    self._container.append(substances.Substance())
...
class Creature(Agent):
    ...
    def pre_actions(self):
        ...
        #исчезает при энергии 0 global

        if  self._energy == 0:
            self.die()
            return False
        #исчезает по рандом при возрасте больше 10 random.random() <= 0.0001 and self.age > 10 or
        if  self.age > 1000:
            self.die()
            return False
    ...
    def can_mate(self, with_who):
        if isinstance(with_who, Creature):
            if with_who.sex != self.sex and  self.age < 600: #self.age > 200 and
    ...
    def will_mate(self, with_who):
        ...
        if self.sex:
            ...
            #    return True
                ...
                #return "true"#random.random() < 3. * partner_has_substance / (self_has_substance*3 + partner_has_substance)


#решение размножаться
