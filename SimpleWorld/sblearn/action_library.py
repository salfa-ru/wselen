...
import math
import random
from . import entities, substances
from . import states

from .visualization import count_b
...
class MovementXY(Action):
    ...
    def do(self):
        ...
        previus_step_x = self.subject.x

        previus_step_y = self.subject.y
        ...
        #current_step_x = previus_step_x + random.randint(-1, 1)
        #current_step_y = previus_step_y + random.randint(-1, 1)
        # current_step_x=current_step_x-1

        #previus_step_x = current_step_x
        #previus_step_y = current_step_y

        self.subject._energy -= 1
        ...
        if self.subject.board.cell_passable(current_step_x, current_step_y):
            ...
            self.subject.count_substance_of_type(substances.Substance)
            self.subject._energy -= 1
            self.subject._count_energy += 1


        if self.subject._count_energy > 10 and len(self.subject._container)>0:
            self.subject._container.pop(0)
            self.subject._count_energy = 0
...
class GiveBirth(Action):
    ...
    def get_empty_cells_around(self):
        ...
        if self.subject.board.cell_passable(self.subject.x - 1, self.subject.y):
            cells_near.append((self.subject.x - 1, self.subject.y))
        if self.subject.board.cell_passable(self.subject.x+1, self.subject.y + 1):
            cells_near.append((self.subject.x+1, self.subject.y + 1))
        if self.subject.board.cell_passable(self.subject.x-1, self.subject.y - 1):
            cells_near.append((self.subject.x-1, self.subject.y - 1))
        if self.subject.board.cell_passable(self.subject.x + 1, self.subject.y-1):
            cells_near.append((self.subject.x + 1, self.subject.y-1))
